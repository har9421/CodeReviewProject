...
namespace CodeReviewRunner.Services
{
    public class AzureDevOpsService : IAzureDevOpsService
    {
                ...
                foreach (var url in urls)
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            {
                                _logger.LogWarning("❌ Failed with {Url}: {StatusCode} - {Error}", url, response.StatusCode,
        ...
        private async Task<object?> GetPullRequestDetailsAsync(
            string organization,
            string project,
            string repositoryId,
            string pullRequestId,
            CancellationToken cancellationToken)
        {
            try
            {
                var url = $"{organization.TrimEnd('/')}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}?api-version={_options.AzureDevOps.ApiVersion}";
